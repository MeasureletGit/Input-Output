...
def verify(type, mass, comment='', container_type=None):
    ...
    try:
        # Læs 'setup.txt' og tjek om den indeholder 'output'
        with open("setup.txt", "r") as f:
            setup_mode = f.read().strip().lower()

        # Tjek grænse afhængigt af setup-mode
        if setup_mode == "output":
            if int(mass) <= 45 and "kropsvægt" not in comment.lower():
                showerror("Fejl", "Måling under 45 gram er ikke tilladt")
                rungui_s()
                return
        else:
...
def new_single_user(navn):
    ...
    backstafftest.place(x=650, y=170, height=60, width=125)
    backstafftest.config(font=('Helvetica', 15))
