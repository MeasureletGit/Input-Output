...
def new_single_user(navn):
    ...
    backstaffpicker = tk.Frame(master=win, bg='#fafafa')
    backstaffpicker.pack(fill=tk.BOTH, expand=1)
    backstaffpicker_label1 = tk.Label(master=backstaffpicker, text='Velkommen', bg='#fafafa', fg='#575756')
    ...
    backstaffpicker_button1 = tk.Button(master=backstaffbottom, text='Startside', command=lambda: welcome(),
                                        bg='#007f93', fg='white', width=25)
    backstaffpicker_button1.grid(row=0, column=0, sticky='NSEW', padx=130, pady=10)
...
def start():
    ...
    backswitch_label1 = tk.Label(master=backstaffpicker, text='Velkommen', bg='#fafafa')
    backswitch_label1.grid(row=0, column=1, sticky='NSEW', padx=10, pady=10)
    backswitch_label1.config(font=("Helvetica", 25))

    backswitch_label2 = tk.Label(master=backstaffpicker, text='Skan Sundhedskort for at starte', bg='#fafafa')
...
def welcome():
    global back, mode, cprn, backstaffpicker
    backstaffpicker.destroy()
    with open('configuration.txt', 'r') as t:
        cn = t.readline().strip()
        mode = cn.split(';')[0]
    ...
    staff = tk.Button(master=back, text='INFORMATION', bg='#007f93', fg='white', command=lambda: runstaff(), height=4,
                      width=25)
...
def iv_measurement(comment=''):
    ...
    backstaffpicker_label4.place(x=120, y=50, height=25, width=450, anchor='w')
...
def runstaff():
    ...
    if mode == 'singleuse':
        ...
        backstaffpicker_label1 = tk.Label(master=backstaffpicker, text='Information', bg='#fafafa', fg='#575756')
        backstaffpicker_label1.grid(row=1, column=0, sticky='NSEW', padx=10, pady=10)
        backstaffpicker_label1.config(font=("Helvetica", 25))
    ...
    if mode == 'singleuse':
        ...
        with open('setup.txt', 'r') as stp:
            ...
            if scale_type not in ('output'):
                ...
                description_label.place(x=285, y=170, height=37, width=120)
                description_label.config(font=("Helvetica", 12, 'bold'))
            else:
                #description_label = tk.Label(master=backstaffpicker, text='Kommentar/ \nKropsvægt' , bg='#ececec', fg='#575756', height=20)
